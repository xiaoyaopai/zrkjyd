<template>
    ...
    <div class="zzry_content">
        ...
        <div class="zzry_c">
            ...
            <ul>
                <router-link to="/zzzs"><li>资质证书</li></router-link>
                <router-link to="/ryzs"><li class="click">荣誉证书</li></router-link>
            </ul>
        </div>
        <div class="zzry_b">
            <ul>
                <li v-for="item in ryzs"  @click="OpenImg($event)">
...
export default {
  ...
  methods:{
      infiniteHandler($state) {
          var len = this.ryzs.length / 6 + 1
          this.axios.get('http://www.people2000.net/mobile/index/advert?classid=14&page='+len).then((response) => {
              let data = response.data.data.data
              let datas = response.data.data.total
              if (data.length) {
                  this.ryzs = this.ryzs.concat(data);
                  $state.loaded();
                  if (this.ryzs.length / datas === 1) {
                      $state.complete();
                  }
              } else {
                  $state.complete();
